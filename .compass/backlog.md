...
- [ ] **Update README.md - it is stale.** Predates the compass CLI, the command-hook architecture (SPEC-004/ADR-005), the token-efficiency design, and the setup/update split. The bootstrap->setup rename was sed'd through but the substance is out of date: it still frames install around copying agents/skills only (no CLI, no hooks-as-command), does not mention `/compass:update` vs `/compass:setup`, and omits the `compass` CLI commands. Rewrite to reflect v0.3.0: what Compass is, the install model (`/compass:setup` first time, `/compass:update` to refresh from git), the CLI's role in keeping bookkeeping off the agent budget, and the current command surface.
