...
- [x] **SubagentStop signals (opened 2026-08-08, closed 2026-08-30 by [[ADR-019-subagentstop-redelivery-and-teammate-typing]]).** The pre-registered observation was run: an inline `Agent`-tool spawn DOES populate `agent_type` (a `debug` probe produced `debug-finished`), so the typed path was never dead - only teammate-style spawns arrive empty, and those are now typed from their name when it carries one. The observation also found the real defect: SubagentStop is delivered twice per completion (same `agent_id`, 9ms apart), doubling every capture file and every strong signal until deduped.
...
- [ ] **Domain-name alignment as a per-repo KPI (the human, 2026-08-30).** His words: "there's redundancy in the folder names - I'm not sure what that redundancy will mean but I think it might be something we could use as a KPI for each repo. like if we have the same folder names on all places." The migrated vault already shows it: distribution, learning, and vault-structure exist in both specs/ and research/. [Related: the taxonomy research's package-by-feature tension - identical domain names across type dirs are the feature axis showing through the type-first layout.] Needs a definition before it can be a KPI; feeds [[specs/vault-structure/SPEC-022-vault-organized-per-domain]] D-11's measurement-first strategy. Candidate meanings (2026-08-30):
  - Coverage-gap detector: a name in one type dir but not another marks a subject missing its evidence, rulings, or need (live: specs/pipeline exists, research has none - and RESEARCH-decision-coverage-impl sits at root hinted `pipeline`).
  - Finder-confusion predictor: shared names cause wrong-type first clicks (live: the SPEC-015 drill miss went to research/rolling-wave twice); weight collisions by drill miss-rate.
  - Subject-first inversion trigger: high alignment is the evidence the vault wants `<subject>/{specs,research,decisions}` layout; the KPI is the tripwire for proposing that restructure at the human's gate.
  - Fleet health line: alignment per repo = how settled its subject vocabulary is; cheap in vault-health.
  - Duplicated-explanation cost: aligned domains explain the same subject once per type dir; KPI x tokens could justify one shared subject definition.
  - Per D-11: pick the meaning empirically - compute alignment on this vault plus two fleet vaults and correlate with drill/probe outcomes once the measure is ruled accurate.
