...
- [ ] **capture-check races itself: concurrent Stop hooks open duplicate opportunities (observed 2026-08-08).** Two opportunities opened 9ms apart in one vault (OPP-...824730Z and ...833334Z) - two capture-check processes both read a no-open-opportunity state and both opened one; capture-state.json has no lock. Fix: an O_EXCL lockfile (or atomic rename) around the read-decide-write window in capturelib, plus a verify that a second concurrent open becomes a no-op; regression test with two racing invocations.
