...
- [x] FLEET DISTRIBUTION v0.6.0 then v0.6.1 (2026-08-08): 45/45 vaults doctor-clean; per-project git commits of Compass-owned paths only; 27-28 pushed per wave. v0.6.1 moves the capture pass into a spawned subagent relaying one summary line (context-pollution fix); duplicate-opportunity race filed in backlog. Not pushed: 7 no-git projects, 2 no-remote, 1 all-paths-gitignored (iwyc-unreal), 4 push-rejected (3 ue5-editor-mcp checkouts + wt-spec056 behind their remotes - need a human pull/rebase decision; pg-jira-exporter's remote no longer exists).
