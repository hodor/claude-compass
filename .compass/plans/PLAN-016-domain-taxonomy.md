...
**This plan was revised twice before approval.** A three-lens adversarial review (hygiene, systems, product: 11 BLOCKERs, 17 AMENDs/NOTEs) caught: unparseable decision lines and citations claiming nothing (both CLI gates failed); Roger's per-folder-index ruling silently narrowed; the record-name defect cross-contaminating same-named domains across type dirs; sync generating the link form D-08 forbade; a migration that grew the root index it existed to shrink, with no hot-path pass bar; self-grading acceptance drills; a section-name collision with the shipped Children convention; and reorganization waiting on human memory (ruled out by SPEC-022 D-07). Roger's D-08 ruling then removed the survivor: the sync-maintained per-folder Children block was a materialized copy of `ls` - "every time you make a change you have to change the file, the folder and this thing" - so listings are now read live from the filesystem and the plan shrank.
...
- [ ] TASK-109: vault-relative record names for nested artifacts (same-named domains across type dirs stop cross-contaminating child counts), in-place `(folder, N children)` count refresh on root-index lines, and the sync loop guard narrowed to exact root-relative paths - complexity: M, files: [plugin/cli/vaultlib.py, plugin/cli/commands/sync.py, plugin/cli/tests/test_sync.py, plugin/cli/tests/test_vaultlib.py], decisions: [SPEC-022-vault-organized-per-domain/D-03, ADR-022-domains-scope-notes-shallow-when-unsure/D-08, ADR-022-domains-scope-notes-shallow-when-unsure/D-10]
