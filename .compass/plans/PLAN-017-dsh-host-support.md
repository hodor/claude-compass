...
author: "planner"
status: complete
summary: "dsh is a native Compass host from the one canonical plugin source (v0.19-0.23): materializers, model routes, doctor, capture worker, global auto-installed bundle, and Wave 6 zero-decision (SPEC-006 D-05) - hosts detected never asked, dsh-first bootstrap via a second bridge mount, both-direction acceptance at 0 errors"
...
- [x] TASK-016: dsh-first bootstrap - a Compass project whose first-ever session is dsh materializes itself: the global bundle mounts a second bridge instance at an absolute machine-level hooks file (`<dsh-home>/compass-bootstrap-hooks.json`) whose SessionStart command runs the generated `compass-bootstrap.py` against a plugin snapshot beside it, materializing the launch-cwd project when it holds `.compass/` without a complete install and exiting untouched otherwise - files: [plugin/cli/hostlib.py, plugin/cli/commands/doctor.py, plugin/cli/tests/test_hostlib.py, plugin/skills/update/SKILL.md], decisions: [SPEC-006-multi-host-agent-cli-support/D-05], lessons: [LESSON-hook-payloads-observe-before-coding, LESSON-scratch-vaults-need-compass-dir]
  - Landed 2026-09-05, 942 tests green. The design survived as sketched once two platform facts surfaced (probes doc, Wave 6 section): dsh's hook sandbox silently blocks writes outside the session workspace - reads and outside executables pass - and the runtime boots bundles from the profile's `file:` source dir, not the node_modules copy `--dump-config` shows. Live: a virgin `.compass`-only project got the full install from its first dsh one-shot; the second session's spec write synced into the index. Every generated command and script is backslash-free (a backslash dies silently in some nested quoting layer between JSON, pwsh, and python).
- [x] TASK-017: zero-decision acceptance both directions - two fresh projects on the rig, one driven dsh-first and one Claude-Code-first, no prompts, no manual steps, both hosts working in both projects afterwards, validate 0 errors - files: [.compass/research/distribution/RESEARCH-dsh-live-probes.md], decisions: [SPEC-006-multi-host-agent-cli-support/D-05]
  - Landed 2026-09-05; full evidence in the probes doc. Dsh-first: bootstrap, then Claude Code edited the bootstrapped spec and sync fired; validate 0 errors. CC-first: one apply, no roster field, both hosts materialized (the real `perform` network path proved the same); a `claude -p` spec and a dsh research doc each indexed by their own host's hooks; validate clean; doctor OK on every row. v0.23.0.
