...
- **The worker binary resolves through `COMPASS_CLAUDE_BIN`** (env, then config, then PATH `claude`). This makes the unittest stub deterministic and lets the live fire induce the real `no-headless` path on a scratch vault by pointing the override at a failing stub - the same code path an auth-less fleet host takes.
