...
- **D-05:** What "just works" must be: "I just start it via deepseek harness or inside claude code and I don't have to do anything and everything will work on both. we will not settle for anything less than that. the user must not know or be forced to decide anything regarding this, it should just work." No roster question at setup, no one-time step, no ordering requirement between hosts. Host support is detected and materialized automatically - and a project first opened from dsh must bootstrap itself exactly as one first opened from Claude Code does.
