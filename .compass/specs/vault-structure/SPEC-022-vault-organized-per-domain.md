...
- **D-12:** Lessons and ADRs should also be taxonomized the same way. Supersedes the lessons Non-Goal below.
- **D-13:** Agents should be able to grep as well in all indexes in an easy way - that's probably how to minimize waste - making that the most obvious thing for them to do.
- **D-14:** The lesson catalog should follow the same structure as everything: index.md on the root folder explaining the subfolders in overall - or explaining lessons super high level - and then more index.md files on the subfolders; AI agents grep from the main index and go from there finding more files. That has to be the usual structure - that is how the hot path goes down.
- **D-11:** The organizing strategy follows the measurement. Optimize taxonomy/categorization by measuring against the unrelated token consumption of subagents - but first ensure the measurement is 100% accurate, and figure out ways to empirically prove that it is; after that the strategy used to organize is 100% dependent on the efficiency of it.
...
- Lessons - the catalog is their index; they group by tags already. (Superseded by D-12: lessons and ADRs taxonomize the same way.)
