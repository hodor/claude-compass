...
- **D-09:** Petersen's mapping study is the conduct, by elimination: just AI will not be enough to review the quality of a source; that would need humans. Nothing is excluded on quality. Dedicated retrievers per curated source, regardless of what else we do; that makes our lives easier. Every finding carries mechanical marks: evidence type, retraction status, preprint status, citation count, convergence. Evidence type is graded, peer review being really good, but the grade's weight depends on the topic: when doing R&D for AI, where we want to see what is new, peer review is not as critical. Agents grade the papers. The grade is a soft thing, not a hard thing: it is completely fine to use the grade as long as the agent tells the user it plans to use it and the user can tell it NOT to use the grade. Judgment comes in the form of lessons we learn and by finding new papers that prove previous papers wrong.
...
- Source curation: every finding carries mechanical reliability marks, recorded, and a specialist's blog is admitted as a source while an uncurated pile of blogs is not.
- A dedicated retriever per curated source, so reliability lives in where the agent searches.
- A source ledger: lessons about sources that misled a run, and forward citations that overturn a paper, visible to the next run.
