...
The same stall can also present as a full, well-formed compaction summary (Primary Request/Key Concepts/.../Pending Tasks/Optional Next Step) - its thoroughness reads like a finished report, so check its own "Pending Tasks" for a still-undelivered reply before trusting completion status.
Detect it by reading the final message as a contract: if the brief demanded a structured report and the text is progress prose or an unresolved pending item, the work is unfinished no matter what the completion status says.
