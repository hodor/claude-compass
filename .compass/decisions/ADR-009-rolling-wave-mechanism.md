...
**Coverage.** `compass coverage` and `lesson-coverage` treat tasks by state: **detailed** tasks must cite as today (uncovered fails); **intent** lines are `scoped` - reported, never failing, but every spec decision must be claimed by at least one detailed-or-scoped task so nothing silently vanishes; a task that elaborates without its citations is an ordinary uncovered failure at that point. The gate output gains a `scoped` column.

**Record.** The `## Wave N elaborated` sections are the elaboration record: wave outcomes consumed, lines elaborated, deviations from intent with reasons. `compass` needs no new command for v1; the validator's existing plan-vs-implementation audit reads the wave sections.
