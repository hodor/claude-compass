...
- **D-12:** The main agent has its own door into the loop. `compass capture-note "<one sentence>"` files an in-flight observation as evidence under `tmp/subagent-captures/` and records an `agent-noted` strong signal, so the worker judges it at the next Stop; the note passes the same anti-list and dedup as any captured report. Without this door the only manual path was `/compass:learned`, reserved to the human, and an agent that noticed a candidate could only hand the human the command, which is the burden [[SPEC-018-scaffolding-invisible-to-the-human]] D-01 forbids. Observed live 2026-08-24 in daschie-sort: the agent closed a destructive-test incident with "/compass:learned ... that path is yours to invoke". Every skill and rule that named the two creation paths now names the note as the agent's path and forbids suggesting the slash command. (Added 2026-08-24 from the live defect; shipped v0.8.2.)
