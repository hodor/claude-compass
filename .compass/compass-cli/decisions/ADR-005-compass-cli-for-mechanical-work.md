...
- **Fail-safe means never exit 2.** A command hook that exits `2` *blocks the user's write* and shows stderr as a blocking message. `compass sync` must trap every exception and exit `0` (or `1`), never `2`. A bug in sync must never block or destroy Roger's edit (SPEC-004 safe-degrade constraint).
