...
`.compass/lessons/index.md` is the surface agents grep first: domains with counts, loose root lessons, and each domain's own `index.md` listing its lessons with summaries - all regenerated by sync. `.compass/meta/lessons-catalog.yaml` remains the machine index behind `compass lessons` and the single source of truth for what lessons exist; it loads off the hot path.
